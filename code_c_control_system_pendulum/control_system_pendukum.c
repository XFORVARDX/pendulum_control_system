...
int A = 60 ;//амплитуда , потом будем задавать матлабом 
int T = 1 ; // период , потом будем принимать с матлаба 
int error = 0;// для расчета ошибки 
int time12 = 0;
int flag = 0;
...
//                    
//        puts(buffer);   
//        PORTB |= (1 << PORTB5); // Установка высокого уровня на PB5
        byte1 = (ugol >> 8) & 0xFF;
        byte2 = ugol & 0xFF;  
        putchar(0x1c);
        putchar(byte1);   
...
while (1)
    ...
    // result =A*datay[5];          
    if(time12>16128)time12 =0;
    result = A*(int)(fix16_sin[(time12>>6)]>>2);  
    //result = 1966080;
    //result =  A*sin(0.016384*T*time12*2*PI);   
    error = (A*(int)((sin_ugol[ugol>>1]>>2)) -  result);
